    ...
    elseif strcmpi(ClusterMod,'spatial')
       ...
       ci
        ...
        if ((L0TNumSmp*.05) > L1NumC )
          ...
          [L1_Idx , L1_Ctrs] = kmeans(TKmeanFV,L1NumC,'start','cluster','emptyaction','singleton','Options',opts);
        else
            opts = statset('Display','final','MaxIter',1000);
            [L1_Idx , L1_Ctrs] = kmeans(TKmeanFV,L1NumC,'emptyaction','singleton','Options',opts);
              obj = gmdistribution.fit(TKmeanFV,L1NumC,'Regularize',eps,'SharedCov',true , 'CovType','diagonal');
            [L1_Idx,nlogl] = cluster(obj,TKmeanFV) ;
